...
ostream &operator<<(ostream &out, const pair<T1, T2> &x) { return out << x.fi << ' ' << x.se; }
template <typename T1, typename T2>
...
void solution()
{
    ...
    For(t, 1, q)
    {
        ii x, y;
        int k;
        cin >> x.fi >> y.fi >> x.se >> y.se >> k;
        if ((x.se - x.fi) % k || (y.se - y.fi) % k)
        {
            cout << "NO\n";
            continue;
        }
        ...
        if (y.fi >= y.se)
            swap(y.fi, y.se);
        cout << ((T.get(1, 1, m, y.fi, y.se) < x.fi) ? "YES" : "NO") << "\n";
    }
}
/**/
main()
