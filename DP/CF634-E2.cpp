...
void solution()
{
    ...
    //! dp[i][j] - số phần tử j từ 1 -> i
    //! cnt[i] - tổng số phần tử i
    //! g[i] - list vị trí của i
    For(i, 1, n)
    {
        For(j, 1, 200)
        {
    ...
    For(i, 2, n - 1)
    {
        For(j, 1, 200)
        {
            int cur = min(dp[i - 1][j], dp[n][j] - dp[i][j]);
            int l, r;
            if (cur)
                l = g[j][cur - 1], r = g[j][cnt[j] - cur];
            else
                l = 0, r = n + 1;

            if (!r)
                r = n + 1;
            ans = max(ans, dp[r - 1][a[i]] - dp[l][a[i]] + 2 * cur);
        }
    }
    cout << ans << endl;
}
