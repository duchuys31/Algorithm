...
int n, k;
int dp[N][N * 2][2][2];
void solve(int i, int cnt, int up, int down)
{
    if (cnt > k)
        dp[i][cnt][up][down] = 0;
    if (dp[i][cnt][up][down] != -1)
        return;
    if (i == n)
    {
        dp[i][cnt][up][down] = (cnt == k);
        return;
    }
    dp[i][cnt][up][down] = 0;
    For(a, 0, 1)
    {
        For(b, 0, 1)
        {
            if (a != b)
            {
                ...
                {
                    solve(i + 1, cnt + 2, a, b);
                    dp[i][cnt][up][down] += dp[i + 1][cnt + 2][a][b];
                    dp[i][cnt][up][down] %= mod;
                }
                else if (a != up || b != down)
                {
                    solve(i + 1, cnt + 1, a, b);
                    dp[i][cnt][up][down] += dp[i + 1][cnt + 1][a][b];
                    dp[i][cnt][up][down] %= mod;
                }
                else
                {
                    solve(i + 1, cnt, a, b);
                    dp[i][cnt][up][down] += dp[i + 1][cnt][a][b];
                    dp[i][cnt][up][down] %= mod;
                }
            }
            else
